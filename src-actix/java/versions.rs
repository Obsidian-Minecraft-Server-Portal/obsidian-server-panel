...
impl JavaVersion {
    ...
    fn get_executable(&mut self) -> PathBuf {
        ...
        let executable = if cfg!(windows) { path.join("bin/java.exe") } else { path.join("bin/java") };
    ...
    pub async fn install(&self, sender: tokio::sync::mpsc::Sender<actix_web_lab::sse::Event>) -> Result<()> {
        ...
        let keys = files.as_object().ok_or_else(|| anyhow!("Files not an object"))?.keys().cloned().collect::<Vec<_>>();

        let files = Arc::new(files.clone());
        let client = Arc::new(Client::new());
        let progress: Vec<DownloadProgress> = keys.iter().map(|key| DownloadProgress { file: key.clone(), completed: false }).collect();
    ...
    async fn download_installation_file<F>(&self, client: Arc<Client>, files: Arc<Value>, key: &String, callback: F) -> Result<()>
    where
        F: Fn(DownloadProgress) + 'static + Send + Sync,
    {
        let file = files.get(key).ok_or_else(|| anyhow!("File not found"))?;
        let downloads = file.get("downloads").ok_or_else(|| anyhow!("No downloads found"))?;
        let r#type = file.get("type").ok_or_else(|| anyhow!("No type found"))?;
        let is_directory = r#type == "directory";
        let raw = downloads.get("raw").ok_or_else(|| anyhow!("No raw download found"))?;
        ...
        callback(DownloadProgress { file: key.clone(), completed: true });
