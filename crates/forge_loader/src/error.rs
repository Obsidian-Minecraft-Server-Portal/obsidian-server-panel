...
pub enum ForgeError {
    ...
    #[error("Forge API returned error {status}: {message}")]
    Api { status: u16, message: String },

    #[error("No Forge version found for Minecraft {mc_version}")]
    NoVersionForMc { mc_version: String },

    #[error("Java executable not found or not executable: {path}")]
    JavaNotFound { path: String },

    #[error("Forge installer failed with exit code {exit_code}")]
